...
dataFile = fullfile(dataDir,'data_FigS1.mat');

if ~exist(dataFile,'file')
    error(['Required data file not found: %s\n' ...
           'Please run FigS1_1_run.m first, or download the precomputed data.'], ...
           dataFile);
end
%%%%%%%%%%%%%%%%%%%%%%% Plot %%%%%%%%%%%%%%%%%%%%%%%%% 
load(dataFile)
scan = data_FigS1.scan;

%% Plot panel b
figure; hold on;
plot(scan.eta_values, data_FigS1.b.M(1,:),'--k');
plot(scan.eta_values, data_FigS1.b.M(2,:),'--k');
plot(scan.eta_values, data_FigS1.b.M(1,:)+data_FigS1.b.M(2,:),'--r');
plot(scan.eta_values, data_FigS1.b.M(3,:),'b');
title('FigS1b');
...
Md_ave = data_FigS1.c.Md1ave + data_FigS1.c.Md2ave;
Ms_ave = data_FigS1.d.Msave;
