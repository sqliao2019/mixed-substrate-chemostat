...
%%%%%%%%%%%%%%%%%%%%%%% Setting %%%%%%%%%%%%%%%%%%%%%%%%% 
%%%%%% Figure-specific setting (FigS1)

% Substrate Uptake Function  
alpha1_value = 4;
alpha2_value = 3;
K_value = 2;
p0.f1 = @(x) alpha1_value .* x ./(x + K_value);
p0.f2 = @(x) alpha2_value .* x ./(x + K_value);
...
data_FigS1.c.Md1ave = NaN * ones(n1,n2);
data_FigS1.c.Md2ave = NaN * ones(n1,n2);
data_FigS1.d.Msave = NaN * ones(n1,n2);
...
for i = 1:n1
    for j = 1:n2
        ...
        data_FigS1.c.Md1ave(i,j) = 1/2 * (Md1(1:end-1) + Md1(2:end))' * diff(T) / Tperiod; % time average of M1
        data_FigS1.c.Md2ave(i,j) = 1/2 * (Md2(1:end-1) + Md2(2:end))' * diff(T) / Tperiod; % time average of M2
        data_FigS1.d.Msave(i,j) = 1/2 * (Ms(1:end-1) + Ms(2:end))' * diff(T) / Tperiod; % time average of Ms
    end
end


%% Save data
data_FigS1.scan = scan;
save(fullfile(dataDir,'data_FigS1.mat'),'data_FigS1');
